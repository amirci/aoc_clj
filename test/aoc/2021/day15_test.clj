(ns aoc.2021.day15-test
  (:require [aoc.2021.day15 :as sut]
            [clojure.string :as st]
            [clojure.test :as t :refer [deftest is]]))

(def input
  (->> "resources/2021/day15.txt"
       slurp
       st/split-lines
...
(deftest part-1
  (is (= 40 (sut/find-shortest-path sample)))
  (is (= 609 (sut/find-shortest-path input))))
...
(defn repeat-cave-v
  ...
  (vec
    (mapcat
     #(for [row cave] (mapv (partial inc-risk %) row))
     (range 5))))


(deftest part-2
  (let [actual (repeat-cave-v (repeat-cave-h sample))]
    (is (= (count sample-2) (count actual)))
    (is (= sample-2 actual))
    (is (= 315 (sut/find-shortest-path actual))))
  (is (= 315 (sut/find-shortest-path sample-2)))
  (let [actual (repeat-cave-v (repeat-cave-h input))]
    (is (= 2925 (sut/find-shortest-path actual)))))
